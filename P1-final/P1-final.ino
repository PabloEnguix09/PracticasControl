...
float K_p = 3.833; //3,833
float T_i = 560; //560
float T_d = 0.0000002; //0.002
float K_i = K_p / T_i;
float K_d = K_p * T_d;
...
void task_loopcontr(void* arg) {
    ...
    while(1) {
      if(start_stop == 1){
        ...
        float vp = K_p * error;
        float vi = vi_ant + K_i * BLOQUEO_TAREA_LOOPCONTR_MS * error;
        float vd = K_d/BLOQUEO_TAREA_LOOPCONTR_MS * (error - error_ant);
